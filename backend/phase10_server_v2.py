"""
Phase 10 Server v2 - 兼容前端 API 格式
"""
    ...
    def do_POST(self):
        ...
        if self.path == '/move':
            try:
                ...
                # 初始化或更新 AI
                if ai_instance is None or ai_instance.width != size:
                    ai_instance = VirtualSnakeAI(size, size)
            ...
            result = {'status': 'ok', 'game_id': 'phase10'}
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
...
if __name__ == '__main__':
    ...
    print("Phase 10 Server v2 running on port 8089...")
