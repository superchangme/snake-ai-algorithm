"""
Phase 11 Hybrid v3 - 混合策略（修复版）

偶数网格：纯哈密顿回路（100% 满分保证）
奇数网格：BFS + 虚拟蛇模拟

修复：当哈密顿下一步被阻挡时，等待蛇尾移动
"""
...
def run_benchmark(n=10, w=8, h=8):
    print(f"\n{'='*50}")
    print(f"Hybrid AI v3 - {w}x{h} ({'偶数' if w%2==0 and h%2==0 else '奇数'})")
    print(f"{'='*50}")
    results = []
    steps_list = []
    perfect = 0
