...
class HybridAI:
    ...
    def _simple_follow_tail(self, snake):
        """极简跟随尾巴 - 找到第一个安全的方向"""
        head = snake[0]
        ...
        for d in DIRS:
            ...
            if len(new_snake) >= 4:
                if not self._can_reach(nxt, new_snake[-1], set(new_snake[:-1])):
                    continue
    ...
    def get_direction(self, snake, food, grid=None):
        ...
        # 奇数网格
            ...
            # 跟随尾巴
            return self._simple_follow_tail(snake)
...
def run_benchmark(n=10, w=8, h=8):
    print(f"\n{'='*50}")
    print(f"Hybrid AI v5 - {w}x{h} (极简版)")
    print(f"{'='*50}")
    results = []
    steps_list = []
    perfect = 0
