"""
Phase 11 Hybrid v6 - DynamicHamilton + Fallback

改进：
1. 被困时切换到 BFS 主动找食物（不是绕圈子）
2. 多步安全检查
"""
...
class HybridAI:
    ...
    def _find_best_direction(self, snake):
        """找最佳方向：优先能到食物"""
        head = snake[0]
        ...
        # 找所有空格
        empty = [(x, y) for x in range(self.width) for y in range(self.height) if (x, y) not in body_set]
        ...
        for d in DIRS:
            ...
            # 1. 能到尾巴
            if len(new_snake) < 4 or self._can_reach(nxt, new_snake[-1], new_body):
                can_survive = True
            else:
                # 2. 能到任何食物
                remaining = [p for p in empty if p not in new_body]
                if remaining:
                    for f in remaining[:5]:  # 检查前5个
                        if self._can_reach(nxt, f, new_body):
                            can_survive = True
                            break
            
            if can_survive:
                # 分数 = 剩余空间
                remaining_count = len([p for p in empty if p not in new_body])
                candidates.append((remaining_count, d))
    ...
    def get_direction(self, snake, food, grid=None):
        ...
        # 奇数网格
            ...
            # 2. 找不到安全吃法，找最佳存活方向
            return self._find_best_direction(snake)
...
def run_benchmark(n=10, w=8, h=8):
    print(f"\n{'='*50}")
    print(f"Hybrid AI v6 - {w}x{h} (改进版)")
    print(f"{'='*50}")
    results = []
    perfect = 0
    for i in range(n):
