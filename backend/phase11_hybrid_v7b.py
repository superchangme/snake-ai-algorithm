...
class HybridAI:
    def __init__(self, width=8, height=8):
        ...
        # 根据网格大小设置阈值
        self.min_free_space = 10 if width <= 7 else 15
        self.max_food_distance = 3 if width <= 7 else 4
    ...
    def _distance_to(self, start, goal, blocked):
        path = self._bfs_path(start, goal, blocked)
        return len(path) - 1 if path else 999
    
    def _count_adjacent_free(self, pos, blocked):
        """计算相邻空格数"""
        count = 0
        for d in DIRS:
            nx, ny = pos[0] + d[0], pos[1] + d[1]
    ...
    def _smart_follow_tail(self, snake):
        ...
        for d in DIRS:
            ...
            # 计算分数：相邻空格越多越好
            free_adj = self._count_adjacent_free(nxt, new_body)
            score = free_adj * 10
    ...
    def get_direction(self, snake, food, grid=None):
        ...
        # 奇数网格
        food_dist = self._distance_to(head, food, body)
        ...
        if food_path and len(food_path) > 1:
            ...
            can_reach_tail = len(virtual_snake) < 4 or self._can_reach(v_head, virtual_snake[-1], v_body)
            free_adj = self._count_adjacent_free(v_head, v_body)
            
            # 阈值判断
            should_eat = (
                food_dist <= self.max_food_distance and 
                can_reach_tail and 
                free_adj >= self.min_free_space
            )
