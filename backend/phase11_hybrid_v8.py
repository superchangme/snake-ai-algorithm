...
class HybridAI:
    ...
    def _is_danger_zone(self, pos):
        """检查是否在危险区域"""
        if not hasattr(self, 'danger_zone'):
            return False
        return pos in self.danger_zone
    ...
    def _follow_tail(self, snake):
        ...
        for d in DIRS:
            ...
            if self._is_valid(nxt) and nxt not in body_without_tail:
                ...
                # 避让危险区域
                in_danger = self._is_danger_zone(nxt)
        ...
        # 奇数网格
            ...
            if food_path and len(food_path) > 1:
                ...
                if len(virtual_snake) < 4 or self._can_reach(virtual_snake[0], virtual_snake[-1], set(virtual_snake[:-1])):
                    ...
                    # 如果食物在危险区域，检查是否值得冒险
                    if self._is_danger_zone(food):
                        # 只有在蛇还短的时候可以去危险区域
                        if len(snake) < self.total * 0.5:
...
def run_benchmark(n=10, w=8, h=8):
    print(f"\n{'='*50}")
    print(f"Hybrid AI v8 - {w}x{h} (中心避让)")
    print(f"{'='*50}")
    results = []
    perfect = 0
    for i in range(n):
